...
export default function Page({ articles, topLikedArticles }) {
    ...
    const storedTheme = localStorage.getItem(`theme_${userContext.user?.id}`);
    ...
    if (!userContext.user) {
        localStorage.removeItem('theme'); // Delete the theme store in localStorage
        document.documentElement.classList.remove('dark'); // Reset the theme with the default value
        if (toggleSwitch) {
            toggleSwitch.checked = false;
        }
    } else if (storedTheme) {
        // If the user is connected and theme stored, we apply the stocked theme
        document.documentElement.classList.toggle('dark', storedTheme === 'dark');
        if (toggleSwitch) {
            toggleSwitch.checked = storedTheme === 'dark';
        }
    } else if (window.matchMedia('(prefers-color-scheme: dark)').matches) {
        // If the theme isn't stored, we use the favorite theme of the system
        document.documentElement.classList.add('dark');
        if (toggleSwitch) {
            toggleSwitch.checked = true;
        }
    ...
    function switchTheme(e) {
        ...
        localStorage.setItem(`theme_${userContext.user?.id}`, isDarkMode ? 'dark' : 'light'); // Utilisez l'ID de l'utilisateur comme clé
